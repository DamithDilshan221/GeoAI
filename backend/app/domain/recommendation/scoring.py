...
def distance_score(distance_m: float, radius_m: float) -> float:
    """§16.4 Distance: 100 × (1 − distance / radius)."""
...
def freshness_score(status_updated_at: datetime, now: datetime, horizon_hours: float) -> float:
    """§16.4 Freshness: 100 × (1 − hours_since_update / horizon)."""
...
def crowd_score(predicted_usage: float, effective_capacity: float) -> float:
    """§16.4 Crowd: 100 × (1 − predicted_usage / capacity), clamped [0,100]."""
...
def rating_score(rating: float | None) -> float:
    """§16.4 Rating: (rating / 5) × 100, neutral 50 when missing."""
...
def suitability_score(
    ...
    """§16.4 Suitability: 100 if preference met or no preference; penalty otherwise.

    Only ``"staff_preferred"`` is recognised as a live preference value
    (maps to the facility's audience == STAFF check).

    ``"wheelchair_accessible"`` is a legacy string retained for API
    compatibility only; since the Facility entity carries no accessibility
    dict in v3 the check always returns 100 (graceful no-op, decision #6).

    Any other value is also treated as no preference (graceful degradation).
    """
    if secondary_preference == "staff_preferred":
        return 100.0 if str(audience).upper() == "STAFF" else penalty
    # wheelchair_accessible and any other unknown preference → no-op
    return 100.0
