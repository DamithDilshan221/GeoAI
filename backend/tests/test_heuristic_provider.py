...
def test_heuristic_provider_tier1_medium(db_session):
    ...
    f = seed_facility(db_session, "F1", c.id, capacity=20)

    # 3 rows, mean = 6.0
    for i, count in enumerate([4, 6, 8]):
        usage_repo.upsert_hourly_record(
            facility_id=f.id, record_date=date(2026, 9, 1+i), hour=9, day_of_week=2,
            usage_count=count, data_source=DataSource.SYNTHETIC
        )

    provider = HeuristicUsageProvider(usage_repo, facility_repo)
    ctx = PredictionContext(facility_id=f.id, category_id=c.id, capacity=20, day_of_week=2, hour=9)
...
def test_heuristic_provider_tier1_high(db_session):
    ...
    f = seed_facility(db_session, "F2", c.id, capacity=20)

    for i in range(8):
        usage_repo.upsert_hourly_record(
            facility_id=f.id, record_date=date(2026, 9, 1+i), hour=9, day_of_week=2,
            usage_count=10, data_source=DataSource.SYNTHETIC
        )
...
def test_heuristic_provider_tier2(db_session):
    ...
    f1 = seed_facility(db_session, "F_Target", c.id, capacity=20)
    f2 = seed_facility(db_session, "F_Peer", c.id, capacity=20)

    # 0 rows for target f1. 3 rows for peer f2.
    for i in range(3):
        usage_repo.upsert_hourly_record(
            facility_id=f2.id, record_date=date(2026, 9, 1+i), hour=9, day_of_week=2,
            usage_count=15, data_source=DataSource.SYNTHETIC
        )

    provider = HeuristicUsageProvider(usage_repo, facility_repo)
    ctx = PredictionContext(facility_id=f1.id, category_id=c.id, capacity=20, day_of_week=2, hour=9)
    res = provider.predict(ctx)
...
def test_heuristic_provider_tier3(db_session):
    ...
    f1 = seed_facility(db_session, "F_Target", c1.id, capacity=20)
    f2 = seed_facility(db_session, "F_OtherCat", c2.id, capacity=20)

    # 0 rows for target facility and category. 1 row for a completely different category.
    usage_repo.upsert_hourly_record(
        facility_id=f2.id, record_date=date(2026, 9, 1), hour=9, day_of_week=2,
        usage_count=50, data_source=DataSource.SYNTHETIC
    )

    provider = HeuristicUsageProvider(usage_repo, facility_repo)
    ctx = PredictionContext(
        facility_id=f1.id, category_id=c1.id, capacity=20, day_of_week=2, hour=9
    )
    res = provider.predict(ctx)
...
def test_heuristic_provider_tier4_and_capacity_fallback(db_session):
    usage_repo = UsageRecordRepository(db_session)
    facility_repo = FacilityRepository(db_session)
    from app.models.category import Category
    ...
    f1 = seed_facility(db_session, "F_NoCap", c.id, capacity=None)

    # No usage records anywhere
    provider = HeuristicUsageProvider(usage_repo, facility_repo)
    ctx = PredictionContext(
        facility_id=f1.id, category_id=c.id, capacity=None, day_of_week=2, hour=9
    )
    res = provider.predict(ctx)
