...
def test_total_stalls_generated_column_computation(db_session: Session) -> None:
    ...
    fac_id = db_session.execute(
        text("SELECT id FROM facilities WHERE name = 'Washroom A'")
    ).scalar()

    row = db_session.execute(
        text("SELECT total_stalls FROM facilities WHERE id = :fid"), {"fid": fac_id}
    ).fetchone()
    assert row is not None
...
def test_total_stalls_generated_column_default(db_session: Session) -> None:
    ...
    fac_id = db_session.execute(
        text("SELECT id FROM facilities WHERE name = 'Washroom B'")
    ).scalar()
...
def test_alembic_downgrade_upgrade() -> None:
    ...
    res3 = subprocess.run(
        [sys.executable, "-m", "alembic", "downgrade", "-2"],
        cwd=backend_dir,
        capture_output=True,
        text=True,
    )
